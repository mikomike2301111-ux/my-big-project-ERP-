...
function cachedRpc(user, fn, args = [], deps = []) {
  ...
  if (hit && Date.now() - hit.time < SERVER_CACHE_TTL) return Promise.resolve(hit.data);
...
function useServer(user, fn, args = [], deps = []) {
  ...
  const [state, setState] = useState(() => cached ? { loading: false, data: cached.data, error: '' } : { loading: true, data: null, error: '' });
  useEffect(() => {
    let alive = true;
    const hit = serverCache.get(cacheKey);
    if (hit) {
      setState({ loading: false, data: hit.data, error: '' });
      if (Date.now() - hit.time < SERVER_CACHE_TTL) return () => {
        alive = false;
      };
    } else {
    ...
    cachedRpc(user, fn, args, deps)
      ...
      .catch(error => {
        if (alive) setState({ loading: false, data: hit?.data || null, error: error.message });
      });
    return () => {
      alive = false;
    };
...
function SettingsPage({ user }) {
  const tabs = ['company', 'users', 'permissions', 'departments', 'warehouses', 'products', 'manufacturing', 'procurement', 'inventory', 'sales', 'finance', 'tax', 'notifications', 'templates', 'automation', 'integrations', 'spreadsheets', 'supabase', 'audit', 'security', 'backup', 'data', 'api', 'health', 'advanced'];
