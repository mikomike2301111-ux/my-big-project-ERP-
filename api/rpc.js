...
function pdfBuffer({ title, metadata, rows }) {
  return new Promise((resolve, reject) => {
    ...
    const drawHeader = () => {
      ...
      doc.fillColor('#344054').fontSize(7.5);
...
async function excelBuffer({ title, metadata, rows }) {
  const workbook = new ExcelJS.Workbook();
  workbook.creator = 'Farmtrack ERP';
  const sheet = workbook.addWorksheet('Report');
  sheet.addRow([title]);
  sheet.addRow([metadata.replace(/\n/g, ' / ')]);
  sheet.addRow([]);
...
const api = {
  ...
  async generateReportExport(user, filters = {}, format = 'CSV') {
    ...
    let extension = 'csv';
    if (fmt === 'Excel') {
      binaryContent = await excelBuffer({ title: report.name, metadata, rows: center.rows });
      ...
      binaryContent = await pdfBuffer({ title: report.name, metadata, rows: center.rows });
      mimeType = 'application/pdf';
      extension = 'pdf';
    } else if (fmt === 'PowerPoint') {
      binaryContent = await pptxBuffer({ title: report.name, metadata, rows: center.rows });
  ...
  adjustInventory(user, row = {}) {
    ...
    if (num(item.quantity) <= reorderLevel && qty < 0) {
      ...
      if (alertEmails.length) {
        deliverEmail(u, 'low_stock', alertEmails, () => EmailService.sendERPNotification({
          to: alertEmails, title: `Low Stock: ${item.productName}`, message: `${item.productName} has ${num(item.quantity)} units remaining (reorder at ${reorderLevel}). SKU: ${item.sku}`,
          module: 'inventory', referenceType: 'inventory', referenceId: item.id, priority: 'high', actionLabel: 'View Inventory', actionUrl: `${EmailService.PLATFORM_URL}/inventory`
  ...
  saveSale(user, row) {
    ...
    if (customerEmail) {
      ...
      deliverEmail(u, 'invoice', customerEmail, () => EmailService.sendInvoiceCreated({
        to: customerEmail, customerName: sale.customerName, invoiceNo: inv?.invoiceNo || saleNo,
        amount: total, dueDate: inv?.dueDate, invoiceId: invoiceId, companyName
      }), { subject: `Invoice ${inv?.invoiceNo || saleNo}`, relatedModule: 'sales', relatedId: id }).catch(() => {});
      deliverEmail(u, 'sales_order', customerEmail, () => EmailService.sendERPNotification({
        to: customerEmail, title: `Order ${saleNo} Confirmed`, message: `Your order of ${ksh(total)} is confirmed. Delivery status: Pending.`,
        module: 'sales', referenceType: 'sale', referenceId: id, actionLabel: 'Track Order', actionUrl: `${EmailService.PLATFORM_URL}/sales/orders`
  ...
  recordCustomerPayment(user, row = {}) {
    ...
    if (inv) {
      ...
      if (customerEmail) {
        deliverEmail(u, 'payment_receipt', customerEmail, () => EmailService.sendPaymentReceived({
          ...
          balance: num(inv.balanceDue || inv.outstanding), invoiceId: inv.id
  ...
  decideLeave(user, id, decision = {}) {
    ...
    if (app.applicantEmail) {
      deliverEmail(u, 'leave_decision', app.applicantEmail, () => EmailService.sendLeaveApproved({
...
const MINIMAL_STATE = {
  ...
  financeManualJournals: [],
  financeManualJournalLines: [],
  financeManualLedger: [],
  financeManualAuditLogs: [],
