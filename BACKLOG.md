...
- [x] **PWA — install as a real app** — new app-install experience, shown once,
      4 seconds after load, only on the login page:
      - `public/manifest.webmanifest` — standalone installable app (name, theme #050505,
        icons 192/512 any + maskable, shortcuts to Dashboard/Sales/Inventory).
      - `public/sw.js` — service worker: network-first navigations with offline app-shell,
        stale-while-revalidate for hashed assets, never intercepts /api or cross-origin.
      - `public/icons/*.png` — generated by `scripts/generate-pwa-icons.js`
        (`npm run pwa:icons`) from `unity-erp-mark.png` (pure-Node PNG encoder, no libs).
      - `index.html` — manifest link, mobile-web-app-capable, apple-touch-icons.
      - `PwaInstallPrompt` in `src/main.jsx` — deferred `beforeinstallprompt` prompt;
        shows once per session (sessionStorage), Install / Not-now / iOS "Add to Home
        Screen" steps; auto-hides when already installed (`display-mode: standalone`,
        `appinstalled`).
      - Verified: full production build passes and manifest/sw/icons are emitted to `dist/`;
        smoke test green.